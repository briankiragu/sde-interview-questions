...
const getClosestTime = (referenceTime: string, givenTime: string) => {
  ...
  Object.entries(probabilities).forEach(([k, v]) => {
    probabilities[k] = Math.abs(
      parseInt(referenceTimes[0], 10) * 60 + parseInt(referenceTimes[1], 10) - v
    );
  });

  const minDifferenceValue = Math.min(...Object.values(probabilities));

  return Object.keys(probabilities).find(
    (key) => probabilities[key] === minDifferenceValue
  );
};
